...
async function mounted() {
    ...
    const taskRef = {
                wednesday: ['eat', 'sleep', 'repeat'],
                thursday: ['eat', 'sleep', 'repeat'],
    }
    const found = await this.firestore.find({name: 'sam'})
    const data = await this.firestore.get(found)
    await this.firestore.delete(found)
    // console.log(data)


    const creation = await this.firestore.create({
        tasks: taskRef,
    })
    // to always be behind ... ... 
    // await this.firestore.addSubDoc({
        // items: [1,2,3]
    // })
    // console.log(this.firestore.getSubDocs())
}
async function mounted1() {
    // console.log(this.firestoreConfig)
    // console.log(this.vuewin)

    // this.localStorage.set('aaa', {a: 12, b: [1,2,3]})
    // console.log(this.localStorage.toJSON())
    // console.log(this.localStorage.get('aaa'))
    // console.log(this.localStorage.toJSON())
    ...
    // await this.firestore.update(id, {
    ...
    // })
    // console.log('updated')
    // console.log(await this.firestore.get(id))
    // await this.firestore.increment(id, 'points')
    // console.log(await this.firestoreConfig.get(id))
}
...
const methods = {
    generateTasks() {
        const dt = datetime()
        const day = dt.day.name
        const taskRef = this.firestore.get('taskRef')
        const tasks = taskRef[day] // array
        this.items = tasks
    },
    submit(answer) {
        const  ref = {
        ...
        this.item.answer = must(ref, answer)
    },
    toggle() {
        deepToggle(this.item, 'answer')
    },
    err(message) {
...
function errorCaptured(err, vm, info) {
    // err: error trace
...
const contextHandle = {
    ...
    arrowKey(dir) {
        switch(dir) {
            ...
            case 'up':
            case 'down':
                throw new Error('invalid key pressed: only left and right allowed')
        }
    },
    letterKey(letter) {
        switch(letter) {
            ...
            default:
                return this.err('invalid key pressed')
        }
    },
    space() {
        this.toggle()
...
const firestoreConfig = {
    // this name is what gets aliased for firestoreConfig
    name: 'MyDailyTracker',
    collectionId: 'MyDailyTracker',
    subCollectionId: 'reports',
    singletons: [
        {
            name: 'tasks',
            fallback: {
